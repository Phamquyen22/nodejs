...
try {
  ...
  exports.getProduct = (req, res, next) => {
    ...
    .then((product) => {
      ...
      // Now, you have the product, and you can find related products
      Products.find({ "productType.main": product.productType.main })
        .then((relatedProducts) => {
          ...
          // You don't need product.save() here because you are not making any changes to the product.
        })
        .catch((relatedProductsError) => {
          // Handle errors when finding related products
          console.error(relatedProductsError);
          res.status(500).send('Internal server error');
        });
    })
    .catch((productError) => {
      // Handle errors when finding the product
  ...
  exports.postAddOrder = async (req, res, next) => {
    try {
        ...
        if (req.session.cart.totalQty) {
            ...
            // Assuming that the User model has a cart field
            // Reset the user's cart to an empty object or a new instance of a Cart model
            req.user.cart = {};
            await req.user.save();
